		...
		for j in range(steps[n][1]):
			if not(ps[n][i][j]==0):
				...
				if wms>(step*thres/100):
					drpt[n].append([ps[n][i][j], [i, j], wms])
print('Done.')
		...
		if wn==[]:
			...
			for wmn in wn:
				if wmn[0]>wm[0]:
					wm=wmn
			if not(wm==w):
				lineswrt.line(parts=[[[xxs[n][i][j], yys[n][i][j]], [xxs[n][wm[1][0]][wm[1][1]], yys[n][wm[1][0]][wm[1][1]]]]])
				lineswrt.record('')
for n in range(len(shapes)):
	for w in drpt[n]:
		wrt2.point(xxs[n][w[1][0]][w[1][1]], yys[n][w[1][0]][w[1][1]])
		wrt2.record('', round(w[0], 2), float(w[1][0]), float(w[1][1]), w[2])
wrt2.save('shapes/drainage_points')
lineswrt.save('shapes/drainage_links')
print('Done.')
